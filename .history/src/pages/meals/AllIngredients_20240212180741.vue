<template>
  <div class="p-8 lg:mx-20">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 md:grid-cols-2 gap-3 gap-6">
      <router-link
        :to="{
          name: 'byIngredient',
        ...
        class="block bg-[#ef4444] text-white p-2 lg:p-3 rounded mb-3 shadow text-center hover:scale-110 transition-all mt-5 text-2xl"
      >
...
export default {
  setup() {
    ...
    function searchMealsByIng() {
      hasIngDesc.value.filter((i) =>
        i.strIngredient.toLowerCase().includes(enterIng.value.toLowerCase())
      );
    }

    return { ingredients, hasIngDesc, enterIng, searchMealsByIng };
  },
};
</script>
