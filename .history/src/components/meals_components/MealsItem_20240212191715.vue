<template>
  <div>
    ...
    <div
      v-if="!meals.length && meals.value == null"
      class="flex justify-center text-gray-600 md:text-3xl"
    >
      There are no meals,,<slot></slot>
    </div>
  </div>
</template>
...
export default {
  ...
  props: ["meals"],
  setup() {
    const nullMeal = computed(() => {
      return store.getters.getNull;
    });
  },
};
</script>
