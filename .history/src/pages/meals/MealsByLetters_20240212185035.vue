<template>
  <div class="lg:mx-20">
    <div class="card flex gap-1 justify-center">
      <router-link
        :to="{ name: 'byLetter', params: { letter } }"
        v-for="letter in letters"
        :key="letter"
        class="bg-[#dc2626] grid grid-cols-1 lg:grid-cols-3 md:grid-cols-2 px-2 rounded text-white hover:scale-150 hover:mx-2 transition-all mt-5 text-2xl"
      >
        {{ letter }}</router-link
      >
    </div>
