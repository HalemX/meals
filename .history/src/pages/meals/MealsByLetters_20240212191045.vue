  ...
  <div class="lg:mx-20">
    <div class="flex justify-center items-center gap-1">
      <router-link
        ...
        :key="letter"
        class="w-fit bg-[#dc2626] px-2 rounded text-white hover:scale-150 hover:mx-2 transition-all mt-5 text-2xl"
      >
        {{ letter }}</router-link
      >
    </div>

    <meals-item :class="{ animate: update }" :meals="filterMeals"
...
export default {
  ...
  setup() {
    ...
    const mealsByLetter = computed(() => {
      filterMeals.value = store.getters.getSearchMealsByLetter.filter(
        (meal) => {
          return meal.length == null;
        }
      );
    });
    ...
    return { letters, mealsByLetter, update, filterMeals };
  },
};
</script>
