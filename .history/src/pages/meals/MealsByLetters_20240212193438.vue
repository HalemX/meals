<template>
  <div class="lg:mx-20">
    <div class="all-letters flex justify-center">
      <div>
        <router-link
          :to="{ name: 'byLetter', params: { letter } }"
          v-for="letter in FirstLetters"
          :key="letter"
          class="inline-block w-fit mx-1 bg-[#dc2626] px-2 rounded text-white hover:scale-150 hover:mx-2 transition-all mt-5 text-2xl"
        >
          {{ letter }}</router-link
        >
      </div>
      <div>
        <router-link
          ...
          :key="letter"
          class="inline-block w-fit bg-[#dc2626] px-2 rounded text-white hover:scale-150 hover:mx-2 transition-all mt-5 text-2xl"
        >
          {{ letter }}</router-link
        >
      </div>
